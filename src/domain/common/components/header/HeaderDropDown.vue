<script setup lang="ts">
  ...
  watch(
    ...
    async ([tripList, newId]) => {
      ...
      console.log(
        'TripList IDs:',
        tripList.map((trip) => ({ id: trip.id, type: typeof trip.id })),
      );
      ...
      if (tripExists) {
        ...
        travelService.getTravelDetail(numericId);
...
<template>
  <div class="header-drop-down-layout">
    ...
    <CreateTripModal :show="showCreateModal" @close="handleModalClose" @create="handleTripCreate" />

    <AcceptLottie ref="acceptLottieRef" title="접근할 수 없습니다" :success="false" />
  </div>
</template>
